...
export class LargeList extends BaseComponent<LargeListPropType> {
    ...
    render() {
        ...
        if (this.props.renderEmpty && (data.length === 0 || data[0].items.length === 0)) {
        ...
        if (this._footerLayout) {
            sumHeight += this._footerLayout.height;
        }
        const contentStyle =
            sumHeight > 0
                ? {
    ...
    _renderHeaderBackground() {
        ...
        if (!renderScaleHeaderBackground || !renderScaleHeaderBackground() || !this._headerLayout) {
            return null;
        }
    ...
    _renderHeader() {
        ...
        return (
            <Animated.View
                style={StyleSheet.flatten([header.props.style, {zIndex}])}
    ...
    _onHeaderLayout = e => {
        if (this._headerLayout && this._headerLayout.height === e.nativeEvent.layout.height) {
            return;
        }
    ...
    _onFooterLayout = e => {
        if (this._footerLayout && this._footerLayout.height === e.nativeEvent.layout.height) {
            return;
        }
